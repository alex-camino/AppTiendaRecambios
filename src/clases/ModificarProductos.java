...
public class ModificarProductos extends JPanel {
	 ...
	 * Create the frame.
	 */
	public ModificarProductos(JPanel contentPane, final Connection conexion, Piezas pieza) {
		  ...
		  ||cantidad.getText().equals("")||precioPieza.getText().equals("")){
			...
			if(!error){
				...
				try{
					...
					PreparedStatement pstmt= conexion.prepareStatement("update piezas set pie_referencia=?, pie_nombre=?, pie_descripcion=?, pie_cantidad=?,"
					+" pie_precio=?, pro_codigo=?, cat_esp_codigo=? where pie_codigo=?");
					...
					pstmt.setInt(8, pieza.obtenerCodigoPieza(conexion));
					pstmt.executeUpdate();
					
					pstmt.close();
					
					//Eliminamos la relacion y la volvemos a crear con los nuevos valores.
					pstmt= conexion.prepareStatement("delete from proveedores_piezas where pro_codigo in (select pro_codigo from proveedores where pro_nombre='"+pieza.getProveedores().getProNombre()+"') and "
					+"pie_codigo="+pieza.getPieCodigo());
					pstmt.executeUpdate();
					
					pstmt.close();
					
					//Creamos la nueva relacion entre proveedor y pieza.
					pstmt = conexion.prepareStatement("insert into proveedores_piezas(pro_codigo,pie_codigo) "
							+"values("+(comboBoxProveedores.getSelectedIndex()+1)+","+pieza.getPieCodigo()+");");
							pstmt.executeUpdate();
	...
	public void rellenarInfo(){
		...
		comboBoxSubcategorias.setSelectedItem(pieza.getCatEspecifica().getCatEspNombre());
		referencia.setText(pieza.getPieReferencia());
		nombrePieza.setText(pieza.getPieNombre());
		descripcionPieza.setText(pieza.getPieDescripcion());
		comboBoxProveedores.setSelectedItem(pieza.getProveedores().getProNombre());
		cantidad.setText(pieza.getPieCantidad().toString());
		precioPieza.setText(pieza.getPiePrecio().toString());
	}
